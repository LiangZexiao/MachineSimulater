...
namespace Simulater3
{
    class Send
    {
        ...
        private static void HttpPost(string url, string postData, out JObject result, string ContentType = "application/json")
        {
        ...
        public static void Post(Machine _machine,string NodeUrl,out bool success,out string errmsg)
        {
            Dictionary<string, object> inputs = new Dictionary<string, object>();
            inputs.Add("machine", _machine);
            inputs.Add("paraslist", _machine.ValuesTable);
            inputs.Add("currtime", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            inputs.Add("modeluenum", _machine.ProductQT);
            JObject result;
            HttpPost(NodeUrl + "/opc", JsonConvert.SerializeObject(inputs), out result);
            success = (bool)result["success"];
            errmsg = (string)result["resmsg"];
        }
    }
}
